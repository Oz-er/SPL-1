...
//---------------------------------------------------------------------------  
    ...
    ofstream outfile("Coral_predictions.csv");
    outfile << "Actual_Label,Predicted_Label,Probability_Score\n"; 

    for(int i=0; i<target_local.size(); i++){
        int actual = target_labels[i];
        
        int predicted = knn_predict(source_aligned, source_labels, target_local[i], k_neighbor);
        double prob = get_knn_prob(source_aligned, source_labels, target_local[i], k_neighbor);
    ...
    cout << "AUC:       " << auc << endl;
    cout << ">>> Predicted labels successfully saved to: Coral_predictions.csv" << endl;
    cout << "------------------------------------------------" << endl;
